...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		if(tokens.size()>=1)
		{
			String lt=tokens.get(tokens.size()-1);
			if(!(lt.endsWith("!")||lt.endsWith(".")||lt.endsWith("?")))
			{
				temp=1;
			}
		}
		for(String s:tokens)
		{
			...
			{
				numWords=numWords+1;
				numSyllables=numSyllables+(countSyllables(s));
			}
		}
		
		numSentences=tokens.size()-numWords+temp;
